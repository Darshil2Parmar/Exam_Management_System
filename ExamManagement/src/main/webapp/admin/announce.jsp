...
.descr:hover,.descr:focus,.bloom:hover,.bloom:focus{
	outline: none;
    box-shadow: 0px 0px 10px 5px pink;
					...
					<div class="col-lg-12">
						...
						<form action="../AnnounceServlet" method="post"
							...
							<div class="makemepiss">
								...
								<div class="henil">
									...
									<div class="bloom">
										Select Date : <input type="date" name="date" class="date-ajay" required/>
									</div>
									<div>
										<input type="text" class="descr" name="descr" placeholder="Enter Announcement Description." required/>
									</div>								
									<h3>Select File</h3>
						...
						<table class="table table-bordered table-hover"
							...
							<tbody>
								<%
								int i = 1;
								AnnounceDao rd = new AnnounceDao(ConnectionProvider.getConnection());
								ArrayList<Announc> ed = rd.getAllAnnounceData();
								for (Announc announc : ed) {
								%>
								<tr>
									<td><%=i%></td>
									<td><%=announc.getDate()%></td>
									<td><%=announc.getDesc()%></td>
									<td><a href="announcement/<%=announc.getFile()%>"
										target="_blank"><%=announc.getFile()%></a></td>
								</tr>
								<%
								i++;
								}
